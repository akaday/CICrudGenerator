...
$path = $target."views/". $folder_member .''. $list_file;
    ...
    <div class=\"box\">
        ...
        <div class=\"box-header row\" style=\"margin-bottom: 10px\">
            ...
            <div class=\"col-md-4 text-right\">
                <?php echo anchor(site_url('". $folder_member .''. $controller."/create'), 'Create', 'class=\"btn btn-primary\"'); ?>";
                    if ($excel == 'create') {
                        $string .= "\n\t\t<?php echo anchor(site_url('". $folder_member .''. $controller."/excel'), 'Excel', 'class=\"btn btn-primary\"'); ?>";
                    }
                    if ($word == 'create') {
                        $string .= "\n\t\t<?php echo anchor(site_url('". $folder_member .''. $controller."/word'), 'Word', 'class=\"btn btn-primary\"'); ?>";
                    }
$string .= "\n\t    </div>
        </div><!-- /.box-header-->
        <div class=\"box-body\">
...
$string .= "\n\t\t    <td style=\"text-align:center\">"
        . "\n\t\t\t<?php "
        . "\n\t\t\techo anchor(site_url('". $folder_member .''. $controller."/read/'.$".$controller."->".$primary."),'<span class=\"fa fa-history\"></span>'); "
        . "\n\t\t\techo ' | '; "
        . "\n\t\t\techo anchor(site_url('". $folder_member .''. $controller."/update/'.$".$controller."->".$primary."),'<span class=\"fa fa-edit\"></span>'); "
        . "\n\t\t\techo ' | '; "
        . "\n\t\t\techo anchor(site_url('". $folder_member .''. $controller."/delete/'.$".$controller."->".$primary."),'<span class=\"fa fa-trash\"></span>','onclick=\"javasciprt: return confirm(\\'Are You Sure ?\\')\"'); "
        . "\n\t\t\t?>"
        . "\n\t\t    </td>";
