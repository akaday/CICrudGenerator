...
$path = $target."controllers/". $folder_member .''.$controller_file;
...
$string .= "\n\n    public function index()
    {   
        \$this->auth->is_member(\$this->router->fetch_class());
...
$string .="\n\n    public function index()
    {   
        ...
        \$element = \$this->member_folder.\"$list\";
        //View
...
$string .= "\n\t    );
            ...
            \$element = \$this->member_folder.\"$read\";
            //View
            template_lib(\$element, \$data, '', \$this->member_folder);

        } else {
            \$this->session->set_flashdata('message', 'Data Tidak Ditemukan');
            redirect(site_url('".$folder_member."$controller'));
        }
    }
    ...
    {   
        ...
        \$data = array(
            'button' => 'Create',
            'action' => site_url('".$folder_member."$controller/create_action'),";
...
$string .= "\n\t);
        ...
        \$element = \$this->member_folder.\"$form\";
        //View
        template_lib(\$element, \$data, '', \$this->member_folder);
    }
...
$string .= "\n\t    );
    ...
    {   
        ...
        if (\$row) {
            \$data = array(
                'button' => 'Update',
                'action' => site_url('".$folder_member."$controller/update_action'),";
$result2 = mysql_query("SELECT COLUMN_NAME,COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='$database' AND TABLE_NAME='$table'");
if (mysql_num_rows($result2) > 0)
{
    while ($row2 = mysql_fetch_assoc($result2))
